...
import { clearCart, removeCartItem, minusCartItems, plusCartItems } from "../redux/actions/cart";
...
function Cart() {
  ...
  const onRemoveCartItem = (id) => {
    if (window.confirm('Вы действительно хотите удалить пиццу?')) {
      disputch(removeCartItem(id));
    }
  };

  const onMinusCartItems = (id) => {
    disputch(minusCartItems(id));

  }

  const onPlusCartItems = (id) => {
    disputch(plusCartItems(id));
            ...
            <div className="content__items">
              {
                addedPizzas.map(obj => <CartItem
                  ...
                  onMinus={onMinusCartItems}
                  onPlus={onPlusCartItems}
                />)
              }
            </div >
